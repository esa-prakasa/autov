...
savedVideoFile = 'unetPusbinRes.avi'
...
while(True) and (frameIdx<2000):
    ...
    imr = np.zeros((IMG_WIDTH,IMG_HEIGHT,3),dtype=np.uint8)

    for i in range(IMG_HEIGHT):
        for j in range(IMG_WIDTH):
            imr[i,j,0] = int(preds_t[0,i,j]*255)
            imr[i,j,1] = int(preds_t[0,i,j]*255)
            imr[i,j,2] = int(preds_t[0,i,j]*255)

    imr2 = cv2.resize(imr,(widthOri, heightOri) , interpolation = cv2.INTER_AREA)
    imRGB = np.zeros((heightOri,widthOri,3),dtype=np.uint8)

    #print("imRGB M:%d  N:%d "%(imRGB.shape[0],imRGB.shape[1])) 
    #print("frame M:%d  N:%d "%(frame.shape[0],frame.shape[1])) 
    ...
    for i in range(heightOri):
        for j in range(widthOri):
            ...
            if (val < 128):
                k = 0
            for k in range(k,3,1):
                imRGB[i,j,k] = frame[i,j,k]
